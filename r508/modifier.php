<?php

// Connexion

require_once("connexion.php");

// Fonctions PHP

require_once("fonctions.php");

// Variables externes

$identifiant=$_GET["identifiant"];
$passe=$_GET["passe"];
$id=$_GET["id"];
$nouvelidentifiant=$_GET["nouvelidentifiant"];
$nouveaupasse=$_GET["nouveaupasse"];

// Variables

$message=($id>0?"Modification de l'utilisateur #".$id:"Création d'un nouvel utilisateur");

// Lecture des données

$objet = $connexion -> query("SELECT * FROM r508_utilisateurs WHERE id='".$id."'")->fetch();
$utilisateur = $connexion -> query("SELECT * FROM r508_utilisateurs WHERE identifiant='".$identifiant."' AND passe='".$passe."'")->fetch();

// Traitements

if (strlen($nouvelidentifiant)>0 && strlen($nouveaupasse)>0)
{	if ($id>0)
	{	$impact = $connexion -> query("UPDATE r508_utilisateurs SET identifiant='".$nouvelidentifiant."', passe='".$nouveaupasse."' WHERE id='".$id."'");
	}
	else
	{	$impact = $connexion -> query("INSERT INTO r508_utilisateurs (identifiant,passe) VALUES ('".$nouvelidentifiant."','".$nouveaupasse."')");
	}
	header("Location: pro.php?identifiant=".($utilisateur["id"]==1?$identifiant:$nouvelidentifiant)."&passe=".($utilisateur["id"]==1?$passe:$nouveaupasse));
	exit();
}
?>

<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<title>R 508</title>
<link href="style.css" type="text/css" rel="stylesheet">
</head>

<body>
<a id="sommet"></a>

<header>
<span class="entete">
<a href="#sommet">MMI.3</a>
R 508 | Cybersécurité
<div>
	Espace pro
	<form action="pro.php" method="post">
	<input type="text" name="identifiant" required="required" placeholder="Indentifiant" value="<?php echo isset($identifiant) && strlen($identifiant) > 0 ? ($identifiant) : ''; ?>"><br>
	<input type="text" name="passe" required="required" placeholder="Mot de passe"<?php echo(strlen($passe)>0?"value=\"".$passe."\"":""); ?>><br>
	<input type="submit" value="Connexion">
	</form>
</div>
</span>
<img alt="MMI Chambéry" width="180" src="data:image/png;base64,">
</header>

<h2>
	<?php echo ($message); ?>
</h2>

	<table>
	<thead>
		<tr>
			<td width="10%">Id</td>
			<td width="20%">Identifiant</td>
			<td width="20%">Mot de passe</td>
			<td width="20%">Rôle</td>
			<td width="30%">Opération</td>
		</tr>
	</thead>
	<tbody>

<?php

	// Inscription du formulaire

	echo ("<tr style='background-color:#bde29e;'>\n");
	echo ("<td class='secondaire'>".$objet["id"]."</td>\n");
	echo ("<td><input type='text' id='nouvelidentifiant' class='input' value='".$objet["identifiant"]."'></td>\n");
	echo ("<td><input type='text' id='nouveaupasse' class='input' value='".$objet["passe"]."'></td>\n");
	echo ("<td class='secondaire'>".($objet["id"]==1?"Super administrateur":"Gestionnaire")."</td>\n");
	echo ("<td><span class='entete'>\n");
	echo ("<a href='#' onclick=\"document.location='?identifiant=".$identifiant."&passe=".$passe."&id=".$objet["id"]."&nouvelidentifiant='+document.getElementById('nouvelidentifiant').value+'&nouveaupasse='+document.getElementById('nouveaupasse').value;\">Valider</a>\n");
	echo ("</span></td>\n");
	echo ("</tr>\n");

?>

	</tbody>
	</table>
</body>
</html>