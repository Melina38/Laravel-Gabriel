<?php
// Sécurisation de la session
$cookieParams = session_get_cookie_params();
session_set_cookie_params([
    'lifetime' => $cookieParams['lifetime'],
    'path' => $cookieParams['path'],
    'domain' => $cookieParams['domain'],
    'secure' => isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off',
    'httponly' => true,
    'samesite' => 'Lax'
]);
session_start();




// Connexion

require_once("connexion.php");

// Fonctions PHP

require_once("fonctions.php");


session_start();

// Variables externes

$identifiant=$_POST["identifiant"];
$passe=$_POST["passe"];
$action=$_POST["action"];
$id=$_POST["id"];

// Variables et constantes

$message="Espace pro";
$superadministrateur=false;
$administrateur=false;

$statuts=array(
	"1" => "Livré",
	"2" => "En attente",
	"3" => "Repris",
	"4" => "Annulé"
);

// Traitements

// Si formulaire vide → retour vers index
if (empty($identifiant) || empty($passe)) {
    header("Location: index.php?erreur=1");
    exit();
}

// Requête préparée pour éviter l'injection SQL
$sql = "SELECT * FROM r508_utilisateurs WHERE identifiant = :identifiant AND passe = :passe";
$stmt = $connexion->prepare($sql);
$stmt->execute([
    ':identifiant' => $identifiant,
    ':passe' => $passe
]);
$resultats = $stmt->fetchAll(PDO::FETCH_ASSOC);

if (sizeof($resultats) == 0) {
    // Identifiant/mot de passe incorrect → retour vers index
    header("Location: index.php?erreur=1");
    exit();
}
// Super administrateur
if ($identifiant === "davidrochaix") {
    $superadministrateur = true;
    $stmt = $connexion->query("SELECT * FROM r508_utilisateurs");
    $resultats = $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// Traitements externes

if (strlen($action)>0)
{	require_once($action);
}

?>

<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<title>R 508</title>
<link href="style.css" type="text/css" rel="stylesheet">
</head>

<body>
<a id="sommet"></a>

<header>
<span class="entete">
<a href="index.php">MMI.3</a>
R 508 | Cybersécurité
<div>
	Espace pro
	<form action="pro.php" method="post">
	<input type="text" name="identifiant" required="required" placeholder="Indentifiant" value="<?php echo isset($identifiant) && strlen($identifiant) > 0 ? h($identifiant) : ''; ?>"><br>
	<input type="text" name="passe" required="required" placeholder="Mot de passe"<?php echo(strlen($passe)>0?"value=\"".$passe."\"":""); ?>><br>
	<input type="submit" value="Connexion">
	</form>
</div>
</span>
<img alt="MMI Chambéry" width="180" src="data:image/png;base64,">
</header>


<h2>
	<?php echo h($message); ?>
</h2>

<?php

if (sizeof($resultats)>0)
{

?>

	<table>
	<thead>
		<tr>
			<td width="10%">Id</td>
			<td width="20%">Identifiant</td>
			<td width="20%">Mot de passe</td>
			<td width="20%">Rôle</td>
			<td width="30%">Opération</td>
		</tr>
	</thead>
	<tbody>

	<?php

		// Affichage des résultats

		foreach($resultats as $resultat)
		{	echo("<tr>\n");
			echo("<td>".h($resultat["id"])."</td>\n");
			echo("<td>".h($resultat["identifiant"])."</td>\n");
			echo("<td>".h($resultat["id"]==1?"********":$resultat["passe"])."</td>\n");
			echo("<td>".h($resultat["id"]==1?"Super administrateur":"Gestionnaire")."</td>\n");
			echo("<td><span class='entete'>\n");
			if ($resultat["identifiant"]==$identifiant || $superadministrateur)
			{	echo("<a href='modifier.php?identifiant=".h($identifiant)."&passe=".h($passe)."&action=modifier.php&id=".$resultat["id"]."'>Modifier</a>\n");
			}
			if ($resultat["identifiant"]!="davidrochaix" && $superadministrateur)
			{	echo("<a href='?identifiant=".h($identifiant)."&passe=".h($passe)."&action=supprimer.php&id=".$resultat["id"]."'>Supprimer</a>\n");
			}
			echo("</span></td>\n");
			echo("</tr>\n");
		}
	?>

	</tbody>
	</table>

<?php
	if ($superadministrateur)
	{	echo ("<span class='entete'><a href='modifier.php?identifiant=".h($identifiant)."&passe=".h($passe)."&action=modifier.php&id=0'>Créer un nouveau profil</a></span>\n");
	}
}

if ($administrateur)
{	echo("<h2>Analyse des données de vente</h2>\n");

	$resultats = $connexion -> query("SELECT * FROM r508_ventes")->fetchAll();

	if (sizeof($resultats)>0)
	{
	//Ventes correspondant au nom et au prénom fourni depuis r508_ventes
	?>

		

		<table>
		<thead>
			<tr>
				<td width="20%">Date et heure</td>
				<td width="15%">Nom</td>
				<td width="15%">Prénom</td>
				<td width="30%">Article</td>
				<td width="5%">Prix</td>
				<td width="15%">État</td>
			</tr>
		</thead>
		<tbody>

	<?php

		foreach($resultats as $resultat)
		{	echo("<tr>\n");
			echo("<td>Le ".date_sql_date_et_heure($resultat["dateheure"])."</td>\n");
			echo("<td>".h($resultat["nom"])."</td>\n");
			echo("<td>".h($resultat["prenom"])."</td>\n");
			echo("<td>".h($resultat["article"])."</td>\n");
			echo("<td>".number_format($resultat["prix"],2,",","")."&nbsp;&euro;</td>\n");
			echo("<td>".h($statuts[$resultat["statut"]])." <span class='entete' style='float:right;'><a href='?identifiant=".$identifiant."&passe=".$passe."&action=annuler.php&id=".$resultat["id"]."'>x</a></span></td>\n");
			echo("</tr>\n");

		}

	?>

		</tbody>
		</table>

<?php

	}
}

?>

</body>
</html>